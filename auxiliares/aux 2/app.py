...
from grafica.transformations import lookAt, perspective, ortho, matmul, translate, rotationX, scale
...
if __name__ == "__main__":
    ...
    def on_key_press(symbol, modifier):
        ...
        if symbol == pyglet.window.key._1 and modifier ==pyglet.window.key.MOD_SHIFT:
            conejo2.scale += 0.2
        elif symbol == pyglet.window.key._2 and modifier ==pyglet.window.key.MOD_SHIFT:
            conejo2.scale -= 0.2
        elif symbol == pyglet.window.key.R and modifier ==pyglet.window.key.MOD_SHIFT:
            conejo2.alpha += np.pi / 6
        elif symbol == pyglet.window.key.RIGHT and modifier ==pyglet.window.key.MOD_SHIFT:
            conejo2.x += 0.2
        elif symbol == pyglet.window.key.LEFT and modifier ==pyglet.window.key.MOD_SHIFT:
            conejo2.x -= 0.2
        elif symbol == pyglet.window.key.UP and modifier ==pyglet.window.key.MOD_SHIFT:
            conejo2.z += 0.2
        elif symbol == pyglet.window.key.DOWN and modifier ==pyglet.window.key.MOD_SHIFT:
            conejo2.z -= 0.2
    ...
    def on_draw():
        ...
        pipeline["transform"] = conejo.get_transform().reshape(16 ,1, order="F")
        bunny_gpu.draw(pyglet.gl.GL_TRIANGLES)
        
        pipeline["transform"] = conejo2.get_transform().reshape(16 ,1, order="F")
        bunny_gpu.draw(pyglet.gl.GL_TRIANGLES)
